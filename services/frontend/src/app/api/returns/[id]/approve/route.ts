...
export async function POST(
    ...
    try {
        const result = await odooClient.adminApproveReturn(parseInt(id, 10), session.value);
        return NextResponse.json(result);
    } catch (error: any) {
        console.error(`[admin/returns/${id}/approve] failed:`, error);
        ...
        return NextResponse.json(
            { error: 'Failed to approve return request', detail: error?.message ?? 'Unknown error' },
            { status }
        );
    }
}