...
export async function POST(
    ...
    try {
        const result = await odooClient.adminRejectReturn(parseInt(id, 10), session.value);
        return NextResponse.json(result);
    } catch (error: any) {
        console.error(`[admin/returns/${id}/reject] failed:`, error);
        ...
        return NextResponse.json(
            { error: 'Failed to reject return request', detail: error?.message ?? 'Unknown error' },
            { status }
        );
    }
}